...
local TranslateMap = {
    ["Junk Yard"] = "Bãi phế liệu (Junk Yard)",
    ["Back Alley"] = "Ngõ hẻm (Back Alley)",
    ["Farmyard"] = "Nông trại (Farmyard)",
    ["Shipyard"] = "Bến tàu (Shipyard)",
    ["Shopping Mall"] = "TTTM (Shopping Mall)",
    ["Lucky Beach"] = "Bãi biển (Lucky Beach)",
    ["Power Plant"] = "Nhà máy điện (Power Plant)",
    ["Car Shop"] = "Cửa hàng xe hơi (Car Shop)",
    ["Museum"] = "Bảo tàng (Museum)",
    ["Authenticator"] = "Phòng thẩm định (Authenticator)",
    ["Club"] = "Quán Bar (Club)",
    ["Energy Drink Shop"] = "Tiệm nước tăng lực (Energy Drink Shop)",
    ["Locksmith"] = "Thợ khóa (Locksmith)",
    ["Repair Shop"] = "Tiệm sửa chữa (Repair Shop)",
    ["Car Customisation"] = "Xưởng độ xe (Car Customisation)",
    ["Grading Store"] = "Tiệm định giá (Grading Store)",
    ["Trailer Store"] = "Cửa hàng xe kéo (Trailer Store)",
    ["Item Cleaning Services"] = "Dịch vụ làm sạch (Cleaning)",
    ["Quck Sell Shop"] = "Cửa hàng bán nhanh (Quick Sell)",
    ["Lake"] = "Hồ nước (Lake)"
}
...
-- ==========================================
-- 2. TẠO CỬA SỔ GIAO DIỆN (UI) + HỆ THỐNG KEY
-- ==========================================
local Window = Rayfield:CreateWindow({
   ...
   KeySettings = {
      ...
      SaveKey = false, -- Tự lưu key để không phải nhập lại lần sau
      GrabKeyFromSite = true, -- Cho phép lấy key trực tiếp từ URL bên dưới
      Key = {"https://pastebin.com/raw/Byp17dHK"} -- Link Pastebin của bạn
   }
})
   ...
   Callback = function(Value)
       ...
       if autoBid then
           task.spawn(function()
               ...
               while autoBid do
                   pcall(function() BidEvent:FireServer() end)
                   task.wait(bidSpeed)
               end
           end)
       end
...
MainTab:CreateDropdown({
   ...
   Callback = function(Option)
       ...
       if choice == "Normal (0.7s)" then
           bidSpeed = 0.7
       elseif choice == "Fast (0.3s)" then
           bidSpeed = 0.3
       elseif choice == "The Flash (0.1s)" then
           bidSpeed = 0.1
       end
   end,
})
...
MainTab:CreateButton({
   Name = "🏠 Dịch chuyển về: " .. LocalPlayer.Name .. " House",
   Callback = function()
       local found = false
       pcall(function()
       ...
       if not found then 
           Rayfield:Notify({Title = "Lỗi", Content = "Không tìm thấy nhà của bạn trong Server!", Duration = 3}) 
       end
   end,
})

MainTab:CreateSection("Bản đồ & Yêu thích")
local selectedFav = ""
local favoriteLocations = {}

MainTab:CreateDropdown({
   Name = "Chọn địa điểm để Teleport",
   ...
   Callback = function(Option)
       ...
       if targetVector then
           ...
           if char and char:FindFirstChild("HumanoidRootPart") then
               ...
               Rayfield:Notify({Title = "Thành công!", Content = "Đã đến: " .. selectedFav, Duration = 2})
           end
       end
   end,
})
...
local FavDropdown = MainTab:CreateDropdown({
   Name = "⭐ Danh sách Yêu Thích (Tele Nhanh)",
   Options = {"Chưa có mục yêu thích"},
   CurrentOption = {""},
   MultipleOptions = false,
   Flag = "Dropdown_Favorites",
   Callback = function(Option)
       local favName = Option[1]
       local engKey = NameToEnglishKey[favName]
       local targetVector = LocationList[engKey]
       if targetVector then
           local char = LocalPlayer.Character
           if char and char:FindFirstChild("HumanoidRootPart") then
               char.HumanoidRootPart.CFrame = CFrame.new(targetVector + Vector3.new(0, 3, 0))
               Rayfield:Notify({Title = "Yêu thích", Content = "Đã bay đến: " .. favName, Duration = 2})
           end
       end
   end,
})
...
local flySpeed = 50
PlayerTab:CreateToggle({
   Name = "Bật Bay (Fly - Dùng WASD + Space)",
   CurrentValue = false,
   Flag = "Toggle_Fly",
   Callback = function(Value) flyEnabled = Value end,
})
PlayerTab:CreateSlider({
   Name = "Điều chỉnh Tốc độ Bay",
   Range = {20, 500},
   Increment = 1,
   Suffix = "Fly Speed",
   CurrentValue = 50,
   ...
   Callback = function(Option)
       ...
       if targetPlayer and targetPlayer.Character and targetPlayer.Character:FindFirstChild("HumanoidRootPart") then
           ...
           if char and char:FindFirstChild("HumanoidRootPart") then
               ...
               Rayfield:Notify({Title = "Thành công!", Content = "Đã tele tới " .. targetName, Duration = 2})
           end
       end
   end,
})
...
if not isfolder("UltimateFarmHub") then
    makefolder("UltimateFarmHub")
end
if not isfolder("UltimateFarmHub/Autoload") then
    makefolder("UltimateFarmHub/Autoload")
end
...
local function getConfigFiles()
    ...
    if #files == 0 then table.insert(files, "Trống") end
    return files
end

local selectedConfigToLoad = ""
local ConfigDropdown = ConfigTab:CreateDropdown({
   Name = "Chọn File Config có sẵn",
   Options = getConfigFiles(),
   CurrentOption = {"Trống"},
   MultipleOptions = false,
   Flag = "Dropdown_Configs",
   Callback = function(Option)
...
ConfigTab:CreateButton({
   Name = "🔄 Làm mới danh sách File",
   Callback = function()
       ConfigDropdown:Refresh(getConfigFiles(), true)
       Rayfield:Notify({Title = "Thành công", Content = "Đã cập nhật danh sách file!", Duration = 2})
   end,
})
...
ConfigTab:CreateButton({
   ...
   Callback = function()
       ...
       if isfile(path) then
           Rayfield:Notify({Title = "Lỗi", Content = "File này đã tồn tại! Hãy dùng nút Ghi Đè.", Duration = 3})
       else
           local data = {
           ...
           Rayfield:Notify({Title = "Thành công", Content = "Đã tạo config: " .. configFileName, Duration = 2})
           ConfigDropdown:Refresh(getConfigFiles(), true)
       end
   end,
})
...
ConfigTab:CreateButton({
   Name = "💾 Ghi Đè (Đè dữ liệu vào file đang chọn)",
   Callback = function()
       ...
       writefile(path, HttpService:JSONEncode(data))
       Rayfield:Notify({Title = "Thành công", Content = "Đã ghi đè dữ liệu vào: " .. targetFile, Duration = 2})
   end,
})

ConfigTab:CreateButton({
   Name = "🗑️ Xóa Config Đang Chọn",
   Callback = function()
       if selectedConfigToLoad == "" or selectedConfigToLoad == "Trống" then
           Rayfield:Notify({Title = "Lỗi", Content = "Chưa chọn file để xóa!", Duration = 2})
           return
       end
       ...
       if isfile(path) then
           ...
           if isfile(autoPath) then delfile(autoPath) end
           
           Rayfield:Notify({Title = "Thành công", Content = "Đã xóa config: " .. selectedConfigToLoad, Duration = 2})
           ConfigDropdown:Refresh(getConfigFiles(), true)
       end
   end,
})
...
ConfigTab:CreateButton({
   Name = "⚡ Bật Auto Execute cho File Này",
   Callback = function()
       if selectedConfigToLoad == "" or selectedConfigToLoad == "Trống" then
           Rayfield:Notify({Title = "Lỗi", Content = "Vui lòng chọn file trong danh sách để bật Auto Execute!", Duration = 3})
           return
       end
       ...
       Rayfield:Notify({Title = "Thành công", Content = "Đã bật Auto Execute cho: " .. selectedConfigToLoad, Duration = 3})
   end,
})

ConfigTab:CreateButton({
   Name = "❌ Tắt Auto Execute của File Này",
   Callback = function()
       if selectedConfigToLoad == "" or selectedConfigToLoad == "Trống" then
           Rayfield:Notify({Title = "Lỗi", Content = "Chưa chọn file!", Duration = 2})
           return
       end
       local autoPath = "UltimateFarmHub/Autoload/" .. selectedConfigToLoad .. ".lua"
       if isfile(autoPath) then
           delfile(autoPath)
           Rayfield:Notify({Title = "Thành công", Content = "Đã tắt Auto Execute cho: " .. selectedConfigToLoad, Duration = 3})
       else
           Rayfield:Notify({Title = "Thông báo", Content = "File này chưa bật Auto Execute trước đó.", Duration = 2})
       end
   end,
})
